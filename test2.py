...
class Stopwatch:
    ...
    def get_current_time(self):
        """Единый метод для получения текущего времени секундомера"""
        if self.running:
    ...
    def reset(self):
        ...
        if current_time == 0.0 and self.saved_time != 0.0:
            ...
            self.set_info("✨ Нет времени для сброса")
    ...
    def lap(self):
        ...
        if not self.running and len(self.laps) > 0:
            ...
            self.set_info(
                f"Всего: {self.format_time(total)}\n"
                f"В среднем: {self.format_time(avg)}"
            )
            return
